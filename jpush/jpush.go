...
// 极光推送推送给单个用户
func JPushSingleNotify(jpushKey, jpushSecret string, platform PlatformType, id, title, content string) (err error) {
	cid := NewCidRequest(1, "")
	...
	if platform == IOS {
		...
		n.Alert = content
		n.SetAndroid(&AndroidNotification{Alert: content, Title: title, AlertType: 7, Intent: map[string]string{"url": ""}, Priority: 0, Category: "", BadgeAddNum: 1, LargeIcon: largeIcon})
	}
	// 构建消息
	//var m jpush.Message
