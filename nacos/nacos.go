...
func BeginNacos(cfg Nacos, grpcCfg GRPC) *NacosMgr {
...
func GetNacosService(serviceName string, cfg Nacos, grpcCfg GRPC) *NacosService {
	ip, port, err := GetNacos(cfg, grpcCfg).FindInstance(cfg.ClusterName, cfg.GroupName, serviceName)
	if err != nil {
		return nil
	}
	return &NacosService{Ip: ip, Port: port, ServiceName: serviceName}
}

func GetNacosGrpcConn(ns *NacosService) (conn *grpc.ClientConn, err error) {
	if ns == nil {
...
// grpc最多重连2次
func grpcConn(ns *NacosService, cnt int) (conn *grpc.ClientConn) {
	if cnt >= 3 {
		return nil
	}
...
// 开始
func GetNacos(cfg Nacos, grpcCfg GRPC) *NacosMgr {
	if nacosMgr == nil {
		mu.Lock()
		defer mu.Unlock()
		if nacosMgr == nil {
