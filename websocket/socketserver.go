...
//
// To serve the built'n javascript client-side library look the `websocket.ClientHandler`.
func New(cfg Config) *SocketServer {
	cfg = cfg.Validate()
	return &SocketServer{
		config:                cfg,
...
//
// To serve the built'n javascript client-side library look the `websocket.ClientHandler`.
func (s *SocketServer) Handler() func(ctx *gin.Context) {
...
func (s *SocketServer) Upgrade(ctx *gin.Context) Connection {
	conn, err := s.upgrader.Upgrade(ctx.Writer, ctx.Request, nil)
	if err != nil {
		websocketErr := ctx.Request.Header.Get("websocketErr")
		errorCode := 503
		if websocketErr == "token_error" {
			errorCode = 507
		} else if websocketErr == "token_expired" {
			errorCode = 508
		} else if websocketErr == "token_empty" {
			errorCode = 506
		}
		fmt.Printf("websocket error: %v\n", err)
		ctx.AbortWithStatus(errorCode) // Status Service Unavailable
...
func (s *SocketServer) addConnection(c *connection) {
	s.connections.Store(c.id, c)
}

func (s *SocketServer) getConnection(connID string) (*connection, bool) {
...
// 在线用户添加
func (s *SocketServer) AddOnlineUser(c *connection) {
	s.onlineUsers.Store(c.uid, c.id)
}

// 获取在线用户
func (s *SocketServer) GetOnlineUser(userID string) (string, bool) {
	cValue, ok := s.onlineUsers.Load(userID)
	if ok == true {
		return cValue.(string), ok
	}
...
// 删除在线用户
func (s *SocketServer) DelOnlineUser(userID string) bool {
...
func (s *SocketServer) handleConnection(ctx *gin.Context, websocketConn UnderlineConnection) *connection {
	// use the config's id generator (or the default) to create a websocket client/connection id

	uid := ctx.GetHeader("uid")
	multiLogin := ctx.GetHeader("multi_login")

	cid := s.config.IDGenerator(ctx)
	//断开旧的连接

	// create the new connection
	c := newConnection(ctx, s, websocketConn, cid, uid)
	...
	if multiLogin != "true" {
		//踢出旧的socket
		var oldsid, err = "", true
		oldsid, err = s.GetOnlineUser(uid)
		if err == true {
			_ = s.Disconnect(oldsid)
		}
...
func (s *SocketServer) OnConnection(cb ConnectionFunc) {
...
func (s *SocketServer) IsConnected(connID string) bool {
...
func (s *SocketServer) Join(roomName string, connID string) {
	s.mu.Lock()
...
func (s *SocketServer) IsJoined(roomName string, connID string) bool {
	s.mu.RLock()
...
func (s *SocketServer) LeaveAll(connID string) {
	s.mu.Lock()
...
func (s *SocketServer) Leave(roomName string, connID string) bool {
	s.mu.Lock()
...
func (s *SocketServer) leave(roomName string, connID string) (left bool) {
...
func (s *SocketServer) GetTotalConnections() (n int) {
...
func (s *SocketServer) GetConnections() []Connection {
...
func (s *SocketServer) GetConnection(connID string) Connection {
...
func (s *SocketServer) GetConnectionsByRoom(roomName string) []Connection {
...
// 用于websocket连接之外的地方调用
func (s *SocketServer) EmitToAll(event string, data interface{}) (err error) {
	message, err := s.messageSerializer.serialize(event, data)
	if err != nil {
		return err
	}
	s.EmitMessageToAll(message)
	return nil
}

func (s *SocketServer) EmitMessageToAll(data []byte) {
	s.connections.Range(func(k, v interface{}) bool {
		_, ok := k.(string)
		if !ok {
			return true
...
func (s *SocketServer) emitMessage(from, to string, data []byte) {
...
func (s *SocketServer) Disconnect(connID string) (err error) {
